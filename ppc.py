...
# ── AUTO-DESHABILITACIÓN DEL MODELO POR BAJO RENDIMIENTO ──
# Si el modelo entrenado empieza a perder seguido (racha de pérdidas) o su
# efectividad a largo plazo cae por debajo del mínimo, se deshabilita el
# gating del ML y el bot sigue operando con la lógica base (equivalente a
# no tener modelo cargado: sin umbrales TIMING_MIN_PROB_Cx ni descarte por
# "alta probabilidad de pérdida"). Las señales se siguen registrando, así
# que el desempeño sin modelo queda trazado. El estado persiste en la DB.
ML_DISABLE_ENABLED     = os.environ.get("ML_DISABLE_ENABLED", "1") == "1"
ML_DISABLE_LOSS_STREAK = int(os.environ.get("ML_DISABLE_LOSS_STREAK", "5"))
ML_DISABLE_WINDOW      = int(os.environ.get("ML_DISABLE_WINDOW", "20"))
ML_DISABLE_MIN_SIGNALS = int(os.environ.get("ML_DISABLE_MIN_SIGNALS", "20"))
ML_DISABLE_MAX_WINRATE = float(os.environ.get("ML_DISABLE_MAX_WINRATE", "0.30"))
...
# ─── UMBRALES ──────────────────────────────────────────────────────────────
HISTORY_MAX   = 200
# Retención extendida SOLO para el panel HTML: hasta 600 rondas (el motor
# de señales y las estadísticas de tendencia siguen usando 200).
HISTORY_STORE_MAX = int(os.environ.get("HISTORY_STORE_MAX", "600"))
...
def score_timing_candidate(nivel_actual: int, intento_global: int, diff: float, fail_count: int = 0) -> float:
    ...
    if timing_model is not None and not ml_disabled:
...
async def check_timing_round_trigger():
    ...
    if is_trained and timing_model is not None and not ml_disabled and mejor_score < umbral:
...
def db_init():
    ...
    CREATE TABLE IF NOT EXISTS level_results (
        ...
        intento_local INTEGER NOT NULL,
        result        TEXT    NOT NULL,
        value         REAL,
        created       TEXT    NOT NULL DEFAULT (datetime('now'))
    );
...
def log_level_result(signal_id: Optional[str], nivel: int, intento_local: int,
                     result: str, value: float):
    """FIX estadísticas: registra SIEMPRE la resolución de cada nivel de señal
    (C1/C2/C3) — win (con el intento local en que ganó) o loss cuando se
    agotaron los intentos del nivel. Una sola fila por nivel resuelto, así
    toda señal procesada en Telegram queda garantizada en su bloque C1/C2/C3,
    sin depender de signal_contexts que puedan quedar 'pending' (p. ej. por un
    reinicio a mitad de señal o un descarte del ML antes de la resolución)."""
    try:
        con = _db()
        con.execute(
            "INSERT INTO level_results(signal_id, nivel, intento_local, result, value) "
            "VALUES(?,?,?,?,?)",
            (signal_id, nivel, intento_local, result, value)
        )
        con.commit()
        con.close()
    except Exception as e:
        logger.warning(f"Error guardando level_results: {e}")
...
def save_state():
    values = {
        ...
        "ml_disabled": "1" if ml_disabled else "0",
    }
    _save_dict(values)
...
def load_state():
    ...
    ml_disabled = (d.get("ml_disabled", "0") or "0") == "1"
    if ml_disabled:
        logger.warning("[ML] 🛑 Modelo deshabilitado por bajo rendimiento (persistido) — operando sin ML")
...
def load_full_history() -> List[float]:
    """Hasta HISTORY_STORE_MAX rondas para el panel HTML (600); el motor de
    señales sigue usando load_history() (200)."""
    try:
        con = _db()
        rows = con.execute(
            "SELECT value FROM history ORDER BY id DESC LIMIT ?", (HISTORY_STORE_MAX,)
        ).fetchall()
        con.close()
        return [r["value"] for r in reversed(rows)]
    except Exception as e:
        logger.warning(f"Error cargando history completa: {e}")
        return []
...
def record_dashboard_attempt(win: bool, nivel: int, intento_local: int) -> None:
    ...
    dashboard_last_resolution = {
        ...
        "max_intentos": get_max_attempts(),
        "ultimo_nivel": get_session_max_signals(),
...
# Auto-deshabilitación por rendimiento: cuando el modelo entrenado pierde
# seguido o deja de ser efectivo a largo plazo, se apaga el gating del ML y
# el bot opera con la lógica base. `ml_disabled` persiste en la DB.
ml_disabled: bool = False
ml_loss_streak: int = 0          # racha de pérdidas con ML activo
ml_recent_results: List[bool] = []  # últimos resultados (ventana ML_DISABLE_WINDOW)
...
def predict_timing(context_features: dict) -> Optional[Dict[str, float]]:
    if timing_model is None or ml_disabled:
        return None
    try:
...
def _ml_disable(motivo: str) -> None:
    """Apaga el gating del ML por bajo rendimiento y avisa por Telegram.
    El bot sigue operando con la lógica base; el estado persiste en la DB."""
    global ml_disabled
    if ml_disabled:
        return
    ml_disabled = True
    save_state()
    logger.warning(f"[ML] 🛑 Modelo de timing DESHABILITADO por bajo rendimiento: {motivo} "
                   f"— el bot opera sin ML (lógica base)")
    try:
        asyncio.get_running_loop().create_task(send_signal_msg(
            "⚠️ <b>MODELO DESHABILITADO</b>\n"
            f"Motivo: {motivo}.\n"
            "El bot sigue operando con la lógica base (sin filtro del modelo entrenado)."))
    except RuntimeError:
        pass

def ml_health_check() -> None:
    """Deshabilita el gating del ML si el modelo entrenado viene perdiendo
    seguido (racha de pérdidas) o su efectividad de largo plazo cayó por
    debajo del mínimo configurado. Solo aplica con el modelo ya entrenado."""
    global ml_disabled
    if not ML_DISABLE_ENABLED or ml_disabled or not is_trained:
        return
    if ml_loss_streak >= ML_DISABLE_LOSS_STREAK:
        _ml_disable(f"racha de {ml_loss_streak} pérdidas consecutivas")
        return
    if len(ml_recent_results) >= ML_DISABLE_MIN_SIGNALS:
        wr = sum(1 for w in ml_recent_results if w) / len(ml_recent_results)
        if wr < ML_DISABLE_MAX_WINRATE:
            _ml_disable(f"efectividad {wr:.1%} en las últimas {len(ml_recent_results)} señales "
                        f"(mínimo {ML_DISABLE_MAX_WINRATE:.0%})")
...
async def resolve_active(value: float, signal_index: int):
    ...
    global sig_retry_msg_id, sig_attempt_values
    global shadow_results_recent
    global sig_ml_used, ml_loss_streak
    ...
    # FIX estadísticas: registrar la resolución de ESTE nivel (C1/C2/C3)
    # ANTES de cualquier otro cambio de estado — una fila por nivel en
    # level_results, para que las stats por bloque coincidan 1:1 con lo
    # procesado/enviado por Telegram (incluidas las señales cuyo contexto
    # quedó 'pending' o que venían de una sesión restaurada tras reinicio).
    log_level_result(sig_signal_id, pending_signal_index, sig_attempt,
                     "win" if win else "loss", value)
    ...
    # Trazar rendimiento del ML en las señales de tiempo emitidas con el modelo
    # activo (gating). Si el entrenado pierde seguido o deja de ser efectivo a
    # largo plazo, ml_health_check() lo deshabilita y el bot sigue con la lógica
    # base, sin destruir el modelo ni los datos de entrenamiento.
    if key == 'timing_3x_5x' and sig_ml_used and not shadow_actual:
        ml_recent_results.append(win)
        if len(ml_recent_results) > ML_DISABLE_WINDOW:
            ml_recent_results.pop(0)
        ml_loss_streak = 0 if win else ml_loss_streak + 1
        ml_health_check()
...
async def emit_signal(value: float, tipo_key: str, label: str, motivo: str,
    ...
    try:
        ...
        sig_ml_used = timing_pred is not None
...
@flask_app.route('/api/signal_level_stats')
def api_signal_level_stats():
    """Totales por nivel de señal (C1/C2/C3). FIX: se alimenta de
    level_results (una fila por nivel resuelto, registrada siempre en
    resolve_active), con fallback al método anterior (signal_contexts) solo
    si la tabla nueva todavía no tiene datos. Así no hay señales procesadas
    en Telegram que falten en su bloque C1/C2/C3."""
    niveles = {1: {"win1": 0, "win2": 0, "loss": 0},
               2: {"win1": 0, "win2": 0, "loss": 0},
               3: {"win1": 0, "win2": 0, "loss": 0}}
    total_nuevo = 0
    try:
        con = _db()
        rows = con.execute(
            "SELECT nivel, intento_local, result FROM level_results"
        ).fetchall()
        con.close()
        for r in rows:
            if r["nivel"] not in (1, 2, 3):
                continue
            st = niveles[r["nivel"]]
            if r["result"] == "win":
                if r["intento_local"] == 1:
                    st["win1"] += 1
                elif r["intento_local"] == 2:
                    st["win2"] += 1
            else:
                st["loss"] += 1
            total_nuevo += 1
    except Exception as e:
        logger.warning(f"Error consultando level_results: {e}")

    # Fallback histórico: si level_results todavía no tiene datos (bot
    # actualizado sobre una DB vieja), contar desde signal_contexts.
    if total_nuevo == 0:
        try:
            con = _db()
            rows = con.execute(
                "SELECT attempt_when_win, result, context_json FROM signal_contexts "
                "WHERE result IN ('win','loss') AND context_json IS NOT NULL"
            ).fetchall()
            con.close()
            for r in rows:
                try:
                    ...
                    continue
                nivel = ctx.get("nivel_actual")
                if nivel not in (1, 2, 3):
                    continue
                if r["result"] == "win":
                    if r["attempt_when_win"] == 1:
                        niveles[nivel]["win1"] += 1
                    elif r["attempt_when_win"] == 2:
                        niveles[nivel]["win2"] += 1
                else:
                    niveles[nivel]["loss"] += 1
        except Exception as e:
            logger.warning(f"Error consultando estadísticas por nivel: {e}")
    return {f"C{n}": v for n, v in niveles.items()}, 200

@flask_app.route('/api/signal_level_stats_weekly')
def api_signal_level_stats_weekly():
    """Estadísticas por semana (calendario ISO), desglosadas por nivel de
    señal C1/C2/C3. Devuelve únicamente las últimas 4 semanas con datos."""
    semanas: Dict[str, Dict[int, dict]] = {}
    try:
        con = _db()
        rows = con.execute(
            "SELECT nivel, intento_local, result, created FROM level_results ORDER BY id DESC"
        ).fetchall()
        con.close()
        for r in rows:
            if r["nivel"] not in (1, 2, 3):
            ...
            dt = None
            try:
                dt = datetime.strptime(r["created"], "%Y-%m-%d %H:%M:%S")
            except (TypeError, ValueError):
                try:
                    dt = datetime.fromisoformat(str(r["created"]))
                except (TypeError, ValueError):
                    continue
            iso = dt.isocalendar()
            key = f"{iso[0]}-S{iso[1]:02d}"
            if key not in semanas:
                semanas[key] = {n: {"win1": 0, "win2": 0, "loss": 0} for n in (1, 2, 3)}
            st = semanas[key][r["nivel"]]
            if r["result"] == "win":
                if r["intento_local"] == 1:
                    st["win1"] += 1
                elif r["intento_local"] == 2:
        ...
        logger.warning(f"Error consultando estadísticas semanales: {e}")
    ultimas = dict(sorted(semanas.items(), reverse=True)[:4])
    return {k: {f"C{n}": v for n, v in niveles.items()} for k, niveles in ultimas.items()}, 200
...
async def cmd_mlstatus(message):
    total = len(ml_recent_results)
    wr = (sum(1 for w in ml_recent_results if w) / total * 100) if total else 0.0
    if ml_disabled:
        estado = "🛑 DESHABILITADO por bajo rendimiento (operando sin ML)"
    elif timing_model is not None:
        estado = "✅ activo"
    else:
        estado = "⏳ sin modelo (fase de recolección/entrenamiento)"
    await bot.reply_to(message,
        "🧠 <b>ESTADO DEL MODELO DE TIMING</b>\n"
        "━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Estado: {estado}\n"
        f"📉 Racha de pérdidas: {ml_loss_streak} (máx {ML_DISABLE_LOSS_STREAK})\n"
        f"📊 Ventana: {total} señales — efectividad {wr:.1f}% "
        f"(mín {ML_DISABLE_MAX_WINRATE:.0%} en {ML_DISABLE_MIN_SIGNALS}+ señales)\n"
        f"🎓 Fase: {'EN VIVO' if is_trained else 'ENTRENANDO'}\n"
        "━━━━━━━━━━━━━━━━━━━━━━━",
        parse_mode='HTML')
...
async def main_async():
    ...
    await bot.set_my_commands([
        ...
        types.BotCommand('mlstatus', '🧠 Estado del modelo de timing'),
    ])
